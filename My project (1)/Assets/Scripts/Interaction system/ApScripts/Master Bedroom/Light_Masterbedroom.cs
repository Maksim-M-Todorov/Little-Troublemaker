...
public class Light_Masterbedroom : MonoBehaviour, IInteractable
{
    ...
    private void Update()
    {
        if (Time.timeScale != 0)
        {
            if (!audioSource.isPlaying && moneyCounter.stateLight_MasterBedroom && !hasPlayed)
            {
                audioSource.Play(0);
                hasPlayed = true;
            }
        ...
        if (Time.deltaTime == 0 || moneyCounter.stateLight_MasterBedroom == false)
        {
            audioSource.Stop();
            hasPlayed = false;
        }
